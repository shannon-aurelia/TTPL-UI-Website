...
const enabled = (value) =>
  !["false", "0", "no", "inactive"].includes(clean(value));
...
async function authorized(request, url, anonKey) {
  ...
  if (
    syncSecret &&
    [
      process.env.WEBSITE_SYNC_SECRET,
...
export async function POST(request) {
  ...
  if (!(await authorized(request, url, anonKey)))
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  let snapshot;
  try {
    snapshot = await sheetSnapshot();
  } catch (error) {
    const message =
      error.message === "Unauthorized"
        ...
        : error.message;
    console.error("[sync-attendance] Sheet snapshot failed", {
      error: String(error),
    });
  ...
  const byEmail = new Map(
    (profiles || []).map((item) => [clean(item.email), item]),
  );
  const byNpm = new Map(
    (profiles || [])
      .filter((item) => item.npm)
      .map((item) => [clean(item.npm), item]),
  );
  const byName = new Map(
    (profiles || [])
      .filter((item) => item.full_name)
      .map((item) => [clean(item.full_name), item]),
  );
  const resolveProfile = (row) =>
    byId.get(row["Account ID"]) ||
    byNpm.get(clean(row.npm || row.NPM)) ||
    byEmail.get(clean(row.email || row.Email)) ||
    byName.get(clean(row.full_name || row["Full Name"]));
  const rosterByNpm = new Map(
    (roster || []).map((item) => [clean(item.npm), item]),
  );
  const rosterByName = new Map(
    (roster || []).map((item) => [clean(item.full_name), item]),
  );
  const resolveRoster = (row) =>
    rosterByNpm.get(clean(row.npm || row.NPM)) ||
    rosterByName.get(clean(row.full_name || row["Full Name"]));
  const result = {
    studentsSynced: 0,
    attendanceSynced: 0,
    plansSynced: 0,
    ...
    if (!profile) {
      ...
      if (!rosterStudent) {
        result.warnings.push(
          `Student not found in roster: ${row["Full Name"] || row.Email || row.NPM}`,
        );
        continue;
      }
      const rosterUpdate = {
        ...
        gmail_email:
          row.Gmail || row["Gmail Email"] || rosterStudent.gmail_email,
  ...
  for (const row of snapshot.plans || []) {
    ...
    const sourceKey =
      row.source_key ||
      `plan-${identityId}-${row.report_group || number}-${row.planned_week_start}`;
    ...
    const payload = {
      ...
      report_label:
        group?.title || `${track.toUpperCase()} Module ${row.module_number}`,
  ...
  for (const row of snapshot.attendance || []) {
    ...
    if (status === "deleted") {
      const { error } = await supabase
        ...
        .delete()
        .eq("source_row_key", row.source_key);
      if (error) result.warnings.push(error.message);
      continue;
    }
    const qnaScore =
      row.qna_score === "" || !Number.isFinite(Number(row.qna_score))
        ? null
        : Number(row.qna_score);
    const payload = {
      ...
      report_label:
        group?.title ||
        `${track.toUpperCase()} Module ${row.module_number} Report`,
    ...
    const { error } = await supabase
      ...
      .upsert(payload, { onConflict: "source_row_key" });
    if (error) result.warnings.push(error.message);
    else result.attendanceSynced += 1;
  }
  ...
  await supabase
    ...
    .insert({
      row_count:
        result.studentsSynced + result.attendanceSynced + result.plansSynced,
      result,
    });
