...
export default function AssistantDashboard() {
  ...
  const load = async () => {
    ...
    const [sessionResult, submissionResult, studentResult] = await Promise.all([
      ...
      supabase.from('submissions').select('*, profiles!submissions_student_id_fkey(full_name,npm,email)').order('submitted_at', { ascending: false }),
  ...
  useEffect(() => {
    ...
    if (!loading && user && profile && !['assistant', 'admin'].includes(profile.role)) router.replace('/portal');
  ...
  return <section className="section dashboard-page">
    ...
    <div className="card table-card"><div className="table-scroll"><table className="dashboard-table"><thead><tr><th>Student</th><th>Week</th><th>Track / module</th><th>Attendance</th><th>Deadline</th><th>Submission</th><th>Review</th></tr></thead><tbody>{rows.map((session) => { const submission = submissions.find((item) => item.session_id === session.id); return <tr key={session.id}><td><b>{session.profiles?.full_name}</b><small>{session.profiles?.npm}</small></td><td>{session.week_number}</td><td><b>{session.track.toUpperCase()} · M{session.module_number}</b><small>{session.report_label || session.report_group}</small></td><td><span className={`attendance-badge ${session.attendance_status}`}>{session.attendance_status}</span>{session.is_makeup && <small>Makeup</small>}</td><td>{session.deadline_at ? new Date(session.deadline_at).toLocaleString('en-GB') : 'Closed'}</td><td>{submission ? <><button className="table-link" onClick={() => openFile(submission.file_path)}><Download size={15}/> Open PDF</button><small>{submission.minutes_late} min late · −{submission.late_penalty}</small></> : 'Not submitted'}</td><td>{submission ? <div className="review-controls"><select value={submission.plagiarism_status || 'pending'} onChange={(event) => updateSubmission(submission.id, { plagiarism_status: event.target.value })}><option value="pending">Plagiarism pending</option><option value="clear">Clear</option><option value="review">Needs review</option></select><input type="number" min="0" max="100" placeholder="Grade" defaultValue={submission.grade ?? ''} onBlur={(event) => updateSubmission(submission.id, { grade: event.target.value === '' ? null : Number(event.target.value) })}/><label className="release-toggle"><input type="checkbox" checked={Boolean(submission.grade_released)} onChange={(event) => updateSubmission(submission.id, { grade_released: event.target.checked })}/> Release</label></div> : '—'}</td></tr>; })}</tbody></table></div></div>
  </section>;
}
